...
// cartridge: type 1B, four 8 KiB RAM banks, CGB flag C0. Every scenario in
// one bench under its own time budget.
`default_nettype none
`timescale 1ns/1ps
module tb_restore_engine_mbc5;
reg clk = 0, clk_io = 0;
always #5 clk = ~clk;
always #7 clk_io = ~clk_io;
wire dry_finished, write_finished;
wire [31:0] dry_errors, write_errors;
restore_engine_case #(.WRITE_ENABLED(0), .MBC5(1)) dry_case(
    .clk(clk), .clk_io(clk_io), .finished(dry_finished), .errors(dry_errors));
restore_engine_case #(.WRITE_ENABLED(1), .MBC5(1)) write_case(
...
initial begin
    ...
    if (dry_errors || write_errors)
        $fatal(1, "MBC5 restore engine failures: dry=%0d enabled=%0d", dry_errors, write_errors);
    $display("TB PASS: tb_restore_engine_mbc5");
    $finish;
end
initial begin
    #2000000000;
    $fatal(1, "MBC5 restore engine watchdog");
end
endmodule
