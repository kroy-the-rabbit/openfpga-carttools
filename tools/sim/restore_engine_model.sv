...
reg [7:0] cart_type = CART_TYPE, ram_size_code = RAM_CODE, rom_size_code = ROM_CODE, cgb_flag = CGB_FLAG;
...
wire [14:0] mismatch_offset;
wire [15:0] save_bytes;
wire reprobe_start;
...
reg [12:0] input_index = 0, backup_addr = 0;
...
restore_engine #(.WRITE_ENABLED(WRITE_ENABLED), .WAKE_CYCLES(12),
    ...
    .save_bytes(save_bytes), .io_start(io_start), .io_op(io_op), .io_done(io_done), .io_failed(io_failed),
...
function automatic [7:0] rom_content(input integer address);
    begin
        if (address == 'h143)
            rom_content = CGB_FLAG;
        else if (address >= 'h134 && address < 'h144)
            rom_content = SYNTH_TITLE[127-(address-'h134)*8 -: 8];
        else case(address)
            'h147: rom_content = CART_TYPE;
            'h148: rom_content = ROM_CODE;
            'h149: rom_content = RAM_CODE;
            'h14C: rom_content = 0;
...
always @(posedge clk) begin
    ...
    if (reset) begin
        ...
        if (bus_delay != 0) bus_delay <= bus_delay - 1;
            ...
            if (accepted_wr) begin
                if (accepted_addr < 'h2000) ram_enabled <= accepted_data[3:0] == 'hA;
                    ...
                    check(ram_enabled && ram_index == ram_writes,
                          "RAM write requires the enabled bank in sequential order");
                    ram[ram_index] <= accepted_data;
                ...
                ram_index = ram_bank * 8192 + (accepted_addr - 'hA000);
                read_value = ram_enabled ? ram[ram_index] : 8'hFF;
        ...
        if (bus_wr && bus_addr >= 'hA000 && bus_addr < 'hC000) begin
            ...
            check(bus_wdata === staged_content(ram_bank * 8192 + bus_addr - 'hA000),
...
task automatic model_io(input [1:0] operation);
    ...
    begin
        ...
        if (operation == 0) begin
            ...
            check(save_bytes == SAVE_BYTES, "engine reports the latched save length to the file service");
            for (w=0; w<SAVE_WORDS; w=w+1) load_word(1,w,staged_word(w));
        end else begin
...
task automatic fresh(input integer number);
    ...
    begin
        ...
        cart_type = CART_TYPE; ram_size_code = RAM_CODE; rom_size_code = ROM_CODE; cgb_flag = CGB_FLAG;
        ...
        metadata[1] = 1;
        metadata[2] = SAVE_BYTES;
        metadata[3] = baseline_save_crc;
        metadata[4] = ROM_BYTES;
        metadata[5] = baseline_rom_crc;
        metadata[6] = (CGB_FLAG << 24) | (ROM_CODE << 16) | (RAM_CODE << 8) | CART_TYPE;
...
initial begin
    ...
    baseline_rom_crc = ~c;
    c = 32'hFFFFFFFF;
    for (k=0; k<SAVE_BYTES; k=k+1) c = add_crc(c,staged_content(k));
    baseline_save_crc = ~c;
    ...
    check(original_reads >= 2*SAVE_BYTES, "two complete original-save passes precede backup");
    ...
    if (WRITE_ENABLED && PART != 1) begin
        ...
        launch_commit();
        wait_complete();
        check(failed && error == 9 && ram_writes == SAVE_BYTES && mismatch_offset == 333,
        ...
        if (MBC3) begin
            ...
            fresh(19);
            preflight();
            ram[SAVE_BYTES-1] = ram[SAVE_BYTES-1] ^ 1;
            launch_commit();
            wait_complete();
