...
| Save restore | experimental MBC1 8 KiB and MBC3 32 KiB implementation. Staging, identity and the recovery backup passed twice on hardware (Pokemon Silver, 2026-09-13); the cartridge write is enabled from that point and not yet exercised. See [restore plan](docs/SAVE-RESTORE-PLAN.md) |
...
**The released core does not write save data to a cartridge.** The
experimental restore engine on the `save-restore-la` branch has a separate
MBC1 and MBC3 writer behind `RESTORE_WRITE_ENABLED` in `core_top.sv`. It was
`0` while file staging, identity checks and recovery backups were qualified on
hardware, and is `1` since 2026-09-13 after two clean preflight passes on
Pokemon Silver. Even then a RAM write needs the whole preflight, a verified
recovery file and a deliberate three-second hold. The gates are in
