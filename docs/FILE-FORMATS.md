...
Game Gear dumps are `.gg` files with allocated `GG0000`-style names,
preserving existing captures. GG header product codes are metadata, not game
titles. See [Game Gear](GAME-GEAR.md); no sidecar is emitted.
...
For the experimental MBC1, MBC3 and MBC5 implementation, use
